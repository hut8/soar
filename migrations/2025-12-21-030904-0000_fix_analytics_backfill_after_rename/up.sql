-- Fix analytics backfill to use renamed aircraft columns
-- This migration re-runs the analytics backfill with correct column names
-- after the devices → aircraft rename (migration 2025-12-11-165518-0000)
--
...
SELECT
    ...
    SUM(CASE WHEN towed_by_aircraft_id IS NOT NULL THEN 1 ELSE 0 END) as tow_flight_count,
...
-- Backfill aircraft_analytics
INSERT INTO aircraft_analytics (aircraft_id, registration, aircraft_model, flight_count_total,
...
GROUP BY f.aircraft_id, a.registration, a.aircraft_model
...
-- Calculate z-scores for aircraft (anomaly detection)
-- Z-score formula: (X - μ) / σ
-- where X is the flight count, μ is the mean, and σ is the standard deviation
WITH stats AS (
    SELECT
        AVG(flight_count_30d) as mean_flights_30d,
        STDDEV_POP(flight_count_30d) as stddev_flights_30d
    FROM aircraft_analytics
    WHERE flight_count_30d > 0
)
UPDATE aircraft_analytics
SET z_score_30d = CASE
    WHEN (SELECT stddev_flights_30d FROM stats) > 0 THEN
        (flight_count_30d - (SELECT mean_flights_30d FROM stats)) / (SELECT stddev_flights_30d FROM stats)
    ELSE 0
END,
updated_at = NOW()
WHERE flight_count_30d > 0;

-- Backfill club_analytics_daily (last 90 days)
...
SELECT
    ...
    SUM(CASE WHEN f.towed_by_aircraft_id IS NOT NULL THEN 1 ELSE 0 END) as tow_count
FROM flights f
JOIN clubs c ON c.id = f.club_id
WHERE f.takeoff_time >= CURRENT_DATE - 90
...
-- Backfill airport_analytics_daily (last 90 days)
-- Departure airports
...
WHERE f.takeoff_time >= CURRENT_DATE - 90
    AND f.takeoff_time IS NOT NULL
    AND f.departure_airport_id IS NOT NULL
...
-- Arrival airports (merge with existing data)
INSERT INTO airport_analytics_daily (airport_id, date, airport_ident, airport_name, departure_count, arrival_count)
SELECT
...
WHERE f.takeoff_time >= CURRENT_DATE - 90
    AND f.takeoff_time IS NOT NULL
    AND f.arrival_airport_id IS NOT NULL
...
-- Migration complete
-- ============================================================================
-- Analytics tables have been backfilled with historical data using correct column names

<script lang="ts">
	...
	import AircraftStatusBoard from '$lib/components/club/AircraftStatusBoard.svelte';
	import type {
		Flight,
		FlightDetails,
		Aircraft,
	...
	// Club aircraft for status board
	let clubAircraft = $state<Aircraft[]>([]);
	let loadingAircraft = $state(true);

	// Members for pilot assignment
	...
	async function loadClubAircraft() {
		...
		try {
			...
			clubAircraft = Object.values(byId);
		} catch (err) {
			logger.error('Error loading club aircraft: {error}', { error: err });
		} finally {
...
<!-- Two-column layout: flights (left) + status board (right) on desktop -->
<div class="flex flex-col gap-6 lg:flex-row">
	...
	<!-- Aircraft Status Board sidebar (desktop: right column, mobile: above flights via horizontal scroll) -->
	{#if userBelongsToClub && !loadingAircraft && clubAircraft.length > 0}

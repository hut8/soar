...
/// Consumer that reads raw Beast messages from NATS JetStream durable queue
///
/// This allows the Beast processing service to consume messages from the durable queue
/// published by the soar-beast-ingest service, ensuring no messages are lost during restarts.
#[derive(Clone)]
...
impl JetStreamConsumer {
    ...
    /// - payload: Vec<u8> - the message content (8-byte timestamp + Beast frame)
    ...
    where
        F: FnMut(Vec<u8>, std::sync::Arc<async_nats::jetstream::Message>) -> Fut,
        Fut: std::future::Future<Output = Result<()>>,
    {
        ...
        while let Some(message) = messages.next().await {
            match message {
                Ok(msg) => {
                    // Get payload as Vec<u8> (binary data)
                    let payload = msg.payload.to_vec();

                    // Validate minimum size (8-byte timestamp + at least 1 byte of frame)
                    if payload.len() < 9 {
                        error!(
                            "Invalid Beast message: too short ({} bytes, expected at least 9)",
                            payload.len()
                        );
                        metrics::counter!("beast.jetstream.invalid_message").increment(1);
                    ...
                    match process_message(payload, msg_arc.clone()).await {
                        Ok(()) | Err(_) => {
                            ...
                            if let Err(e) = msg_arc.ack().await {
                                ...
                                metrics::counter!("beast.jetstream.ack_error").increment(1);
                            } else {
                                processed_count += 1;
                                metrics::counter!("beast.jetstream.consumed").increment(1);
                ...
                Err(e) => {
                    ...
                    metrics::counter!("beast.jetstream.receive_error").increment(1);

...
impl Geocoder {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            user_agent: "SOAR Aircraft Geocoder/1.0 (https://github.com/your-repo/soar)".to_string(),
        }
    }
    ...
    pub async fn geocode_address(&self, address: &str) -> Result<Point> {
        ...
        if results.is_empty() {
            return Err(anyhow!("No geocoding results found for address: {}", address));
        }
        
        let result = &results[0];
        
        let latitude: f64 = result.lat.parse()
            .map_err(|e| anyhow!("Invalid latitude in response: {}", e))?;
        let longitude: f64 = result.lon.parse()
    ...
    pub async fn geocode_address_with_retry(&self, address: &str, max_retries: u32) -> Result<Point> {
...
pub async fn geocode_components(
    ...
    // Add country if provided, defaulting to US if not specified
    let country_str = country.unwrap_or("United States");
...
mod tests {
    ...
    fn test_custom_geocoder() {
        let geocoder = Geocoder::with_settings(
